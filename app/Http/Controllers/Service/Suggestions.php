...
class Suggestions extends Controller
{
    ...
    public function index($page = 1)
    {
        ...
        $records = DB::table('service_suggestions')->where('is_hidden', 'no')->orderBy('create_date', 'desc')->skip($offset)->take(12)->get();
        if(count($records) > 0){
            //格式化数据
    ...
    public function doAdd(Request $request)
    {
        ...
        if($id === false){
	        ...
	        //通过分类id发短信给负责人
	        $manager = DB::table('service_suggestion_types')
		        ->leftJoin('user_manager', 'service_suggestion_types.manager_code', '=', 'user_manager.manager_code' )
		        ->where('service_suggestion_types.type_id', $inputs['type_id'])
	            ->first();
			if(isset($manager->cell_phone) && preg_phone($manager->cell_phone)) {
				Message::send($manager->cell_phone,'征求意见（编号:'.$save_data['record_code'].'）已提交，请及时处理');
			}
    ...
    public function ajax_show(Request $request){
        ...
        if(is_null($record) || count($record)<1){
            ...
            $record_detail = array(
                'record_code'=> $record->record_code,
                'type_id'=> $this->page_data['type_list'][$record->type_id],
                'create_date'=> date('Y-m-d H:i', strtotime($record->create_date)),
                'title'=> $record->title,
                'content'=> trim($record->content),
