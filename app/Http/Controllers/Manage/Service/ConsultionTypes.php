...
class ConsultionTypes extends Controller
{
    var $log_info = array();
    var $page_data = array();
    ...
    public function index($page = 1)
    {
        ...
        $offset = 30;
         $consultionTypes = DB::table('service_consultion_types')
        ...
        if(count($consultionTypes) > 0){
            foreach($consultionTypes as $type){
                $consultionType_list[] = array(
	...
	public function create(Request $request)
    {
        ...
        if(!$node_p['service-consultionTypesMng'] || $node_p['service-consultionTypesMng']!='rw'){
        ...
        $pageContent = view('judicial.manage.service.consultionTypesAdd',$this->page_data)->render();
        json_response(['status'=>'succ','type'=>'page', 'res'=>$pageContent]);
    }
	...
	public function store(Request $request)
    {
	    ...
	    //判断重名
		$sql = 'SELECT `type_id` FROM `service_consultion_types` WHERE `type_name` = "'.$inputs['type_name'].'"';
        $re = DB::select($sql);
        if(count($re) > 0){
        ...
        $rs = DB::table('service_consultion_types')->insert($save_data);
        if($rs === false){
    ...
    public function show(Request $request)
    {
        ...
        $type = DB::table('service_consultion_types')
            ->leftJoin('user_manager', 'service_consultion_types.manager_code', '=', 'user_manager.manager_code')
            ->leftJoin('user_office', 'user_manager.office_id', '=', 'user_office.id')
            ->where('service_consultion_types.type_id', $type_id)
            ->first();
        if(is_null($type)){
        ...
        $pageContent = view('judicial.manage.service.consultionTypesDetail',$this->page_data)->render();
        json_response(['status'=>'succ','type'=>'page', 'res'=>$pageContent]);
    }
    ...
    public function edit(Request $request)
    {
        ...
        $this->page_data['managers'] = $managers;
        $this->page_data['type_detail'] = $type_detail;
        $pageContent = view('judicial.manage.service.consultionTypesEdit',$this->page_data)->render();
        json_response(['status'=>'succ','type'=>'page', 'res'=>$pageContent]);
    }
    ...
    public function doEdit(Request $request)
    {
	    ...
	    //判断重名
		$sql = 'SELECT `type_id` FROM `service_consultion_types` WHERE `type_name` = "'.$inputs['type_name'].'" AND `type_id` != '.$type_id;
        $re = DB::select($sql);
        if(count($re) > 0){
            json_response(['status'=>'failed','type'=>'notice', 'res'=>'已存在名称为："'.$inputs['type_name'].'" 的区域']);
        }
        ...
        $rs = DB::table('service_consultion_types')->where('type_id', $type_id)->update($save_data);
        if($rs === false){
            json_response(['status'=>'failed','type'=>'notice', 'res'=>'修改失败']);
        }
        else{
	...
	public function doDelete(Request $request)
    {
        ...
        $type_id = keys_decrypt($request->input('key'));
        $res = DB::table('service_consultions')->where('type_id', $type_id)->get();
        if(count($res) > 0){
            json_response(['status'=>'failed','type'=>'alert', 'res'=>'该分类下包含数据，不能删除！']);
        }
        $row = DB::table('service_consultion_types')->where('type_id',$type_id)->delete();
        if($row > 0){
            //删除成功后刷新页面数据
            $consultionType_list = array();
	        $pages = '';
