            ...
            <div class="law_body">
                ...
                @if(isset($s_lsfw) && is_array($s_lsfw) && count($s_lsfw)>0)
                    @foreach($s_lsfw as $lsfw)
                        <span onclick="javascript: window.location.href='{{URL::to('list'.'/'.$lsfw['channel_id'].'/1')}}';">{{ $lsfw['channel_title'] }}</span>
                    @endforeach
                @endif
            </div>
        </li>
            ...
            <div class="law_body">
                @if(isset($s_sfks) && is_array($s_sfks) && count($s_sfks)>0)
                    @foreach($s_sfks as $sfks)
                        <span onclick="javascript: window.location.href='{{URL::to('list'.'/'.$sfks['channel_id'].'/1')}}';">{{ $sfks['channel_title'] }}</span>
                    @endforeach
                @endif
            </div>
        </li>
            ...
            <div class="law_body">
                ...
                @if(isset($s_sfjd) && is_array($s_sfjd) && count($s_sfjd)>0)
                    @foreach($s_sfjd as $sfjd)
                        <span onclick="javascript: window.location.href='{{URL::to('list'.'/'.$sfjd['channel_id'].'/1')}}';">{{ $sfjd['channel_title'] }}</span>
                    @endforeach
                @endif
            ...
            <div class="law_body">
                ...
                @if(isset($s_flyz) && is_array($s_flyz) && count($s_flyz)>0)
                    @foreach($s_flyz as $flyz)
                        <span onclick="javascript: window.location.href='{{URL::to('list'.'/'.$flyz['channel_id'].'/1')}}';">{{ $flyz['channel_title'] }}</span>
                    @endforeach
                @endif
            </div>
        </li>
