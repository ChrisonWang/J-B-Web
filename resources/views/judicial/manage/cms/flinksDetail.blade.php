    ...
    <div class="panel-body">
        <form class="form-horizontal" id="flinksEditForm">
            ...
            <div class="form-group">
                <label for="title" class="col-md-1 control-label">名称：</label>
                <div class="col-md-3">
                    <input disabled type="text" class="form-control" id="title" name="title" value="{{ $flinks_detail['title'] }}" placeholder="请输一级分类名称" />
                </div>
            </div>
            <div class="form-group">
                <label for="menu-nodes" class="col-md-1 control-label">子链接：</label>
                <div class="col-md-5">
                        ...
                        <table class="table table-bordered table-hover table-condensed">
                            ...
                            @if(isset($flinks_detail['sub_links']) && $flinks_detail['sub_links'] != 'none' && is_array($flinks_detail['sub_links']))
                                @foreach($flinks_detail['sub_links'] as $sub)
                                    <tr>
                                        <td>
                                            <input disabled type="text" class="form-control" data-key="{{ $sub['key'] }}" name="sub_title[]" value="{{ $sub['title'] }}" placeholder="请输链接名称" />
                                        </td>
                                        <td>
                                            <input disabled type="text" class="form-control" name="sub_link[]" value="{{ $sub['link'] }}" placeholder="请输链接地址" />
                                        </td>
                                    </tr>
                                @endforeach
                            @else
