    ...
    <nav aria-label="Page navigation" >
        <ul class="pagination">
            <li @if($pages['now_page'] - 1 == 0)class="disabled"@endif>
                <a href="javascript: void(0); " @if($pages['now_page'] - 1 >0)onclick="list_page('{{$pages['type']}}', '{{$pages['now_page']-1}}');"@endif data-pageno="{{ $pages['now_page'] - 1 }}" aria-label="Previous">
            ...
            <li @if($pages['now_page'] == $pages['count_page'])class="disabled"@endif>
                <a href="javascript: void(0); " @if($pages['now_page'] < $pages['count_page'])onclick="list_page('{{$pages['type']}}', '{{$pages['now_page']+1}}')"@endif data-pageno="{{ $pages['now_page'] + 1 }}" aria-label="Next">
